...
class ReviewsViewModel @Inject constructor(
    ...
    fun sortByMostLiked() {
        val sorted = _uiState.value.reviews.sortedByDescending { it.numLikes }
        _uiState.update { it.copy(reviews = sorted) }
    }

    fun sortByMostRecent() {
        val sorted = _uiState.value.reviews.sortedByDescending { it.fecha } // usa el campo que tengas
        _uiState.update { it.copy(reviews = sorted) }
    }
